...
# Objective Function: Rastrigin Function (1D)
...
def gwo_1d_multimodal(search_agents_no, max_iter, lb, ub):
    alpha_pos = float('inf')
...
if __name__ == "__main__":
    # Challenge: Search in [-5.12, 5.12] where there are many local optima
    print("Testing on Rastrigin Function (Multimodal)")
    lb, ub = -5.12, 5.12
    best_x, best_score, curve = gwo_1d_multimodal(search_agents_no=20, max_iter=50, lb=lb, ub=ub)
    print("\n--- Optimization Result ---")
    ...
    ax1.plot(x, y, linewidth=1, label='Rastrigin Function')
    ax1.scatter(0, objective_function(0), color='red', s=100, zorder=5, label='Global Optimum (0,0)')
    ax1.set_title(f'Search Space: Multimodal (Rastrigin)')
    ax1.set_xlabel('x')
    ax1.set_ylabel('f(x)')
    ax1.legend()
    ax1.grid(True)
    ...
    plt.tight_layout()
    plt.savefig('pict/multimodal_combined.png')
    print("Plot saved to pict/multimodal_combined.png")
